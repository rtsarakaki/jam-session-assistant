...
- **`public.profiles`** — uma linha por utilizador (`id` = `auth.users.id`): **`username`**, **`avatar_url`** (foto HTTPS a partir de `raw_user_meta_data`, como o avatar OAuth no header), `display_name`, bio, **`instruments` (`text[]`)**; insert/update só da própria linha; leitura também entre utilizadores autenticados (Friends / jam). Trigger `handle_new_user_profile` + migrações de backfill.
